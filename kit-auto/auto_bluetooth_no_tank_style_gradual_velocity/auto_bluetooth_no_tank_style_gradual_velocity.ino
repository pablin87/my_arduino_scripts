...
int DELAY = 50;
enum STATE { STOP, MOVING_FORWARD, MOVING_BACKWARD};
STATE state = STOP;
enum TURN_STATE { NO_TURNING, TURNING_RIGHT, TURNING_LEFT};
TURN_STATE turn_state = NO_TURNING;
...
void girar_izquierda(){
  ...
  analogWrite(enA, current_vel);  
  analogWrite(enB, current_vel - current_turn_vel);  
}

void girar_derecha(){
  increase_turn_current_velocity();
  if (turn_state != TURNING_RIGHT){
    current_turn_vel = VEL_ZERO;
  }
  turn_state = TURNING_RIGHT;
  Serial.println("Girando a la Derecha: " + velocity_to_string(current_turn_vel));
  analogWrite(enA, current_vel - current_turn_vel);  
  analogWrite(enB, current_vel);  
}
